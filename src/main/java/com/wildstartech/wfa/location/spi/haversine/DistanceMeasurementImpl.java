...
/**
 * Implementation of the DistanceMeasurement interface.
 * 
 * <p>Internally, this class stores all measurements as meters.  When invoking
 * the <code>setMeasurement(double) method, the class assumes the caller is 
 * passing a value that is in the unit stored in the <code>units</code>
 * property.  I.e., if the <code>units</code> field has a value of 
 * <code>UNITS.IMPERIAL</code> and the <code>setMeasurement(15)</code> method
 * is invoked, the class will assume that the number <code>15</code> is 
 * miles.  Internally, the system will store a value of 24140.2 (because
 * 15 miles * 1,609.34 meters per mile = 24,140.2 meters.</p>
 * 
 * @author Derek Berube, Wildstar Technologies, LLC.
 * @version 0.1, 2015-06-25
 *
 */
public class DistanceMeasurementImpl implements DistanceMeasurement {
  ...
  private UNITS units=getDefaultUnitOfMeasurement();
  ...
  /** 
   * Constructor taking the measurement and units as parameters.
   */
  ...
  public void setMeasurement(double measurement) {
    ...
    if (measurement < 0) {
      ...
      if (this.units == UNITS.METRIC) {
        this.measurement = measurement * 1000;
      } else if (this.units == UNITS.IMPERIAL) {
        // Convert from imperial units to metric units.
        this.measurement = measurement * 1609.34;
      } else {
        // Convert from nautical miles to metric units.
        this.measurement = 1852 * measurement;
      } // END if (this.units == UNITS.METRIC)      
  ...
  public String toString() {
    ...
    sb.append("measurement: ").append(fmt.format(getMeasurementAs(units)));
    sb.append(", units: ");
    switch (units) {
      case METRIC:
  ...
  public String toFormattedString() {
    ...
    units=getUnits();
    fmt=new DecimalFormat("#.##");
    sb.append(fmt.format(getMeasurementAs(units)));
    sb.append(" ");
    switch(units) {
      case METRIC:
